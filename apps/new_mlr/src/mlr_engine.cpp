...
void MLREngine::ReadData() {
  ...
  } else if (read_format_ == "ext_sparse_feature") {
    petuum::ml::ReadDataLabelExtSparseFeatureBinary(train_file, feature_dim_,
        num_train_data_, &train_features_, &train_labels_, feature_one_based_,
        label_one_based_, snappy_compressed_);
    LOG(INFO) << "first: " << train_features_[0]->ToString();
    LOG(INFO) << "last: " << train_features_[train_features_.size()-1]->ToString();
    if (perform_test_) {
      LOG(INFO) << "Reading test file: " << FLAGS_test_file;
      petuum::ml::ReadDataLabelExtSparseFeatureBinary(FLAGS_test_file,
          feature_dim_, num_test_data_, &test_features_, &test_labels_,
          feature_one_based_, label_one_based_, snappy_compressed_);
    }
  } else {
    LOG(FATAL) << "unrecognized format: " << read_format_;
  }
}
...
void MLREngine::Start() {
  ...
  int loss_table_staleness = FLAGS_loss_table_staleness;
...
std::string MLREngine::PrintOneEval(int32_t ith_eval) {
  ...
  const auto& loss_row =
    loss_table_.Get<petuum::DenseRow<float> >(ith_eval, &row_acc);
...
std::string MLREngine::PrintAllEval(int32_t up_to_ith_eval) {
  ...
  for (int i = 0; i <= up_to_ith_eval; ++i) {
    ...
    //const auto& loss_row = row_acc.Get<petuum::DenseRow<float> >();
    const auto &loss_row =
      loss_table_.Get<petuum::DenseRow<float> >(i, &row_acc);
    std::string test_info;
    if (perform_test_) {
      CHECK_LT(0, static_cast<int>(loss_row[kColIdxLossTableNumEvalTest]));
      std::string test_zero_one_str =
...
std::string MLREngine::GetExperimentInfo() const {
  ...
  ss << "Train set: " << FLAGS_train_file << std::endl
    ...
    << "num_worker_threads: " << FLAGS_num_table_threads << std::endl
