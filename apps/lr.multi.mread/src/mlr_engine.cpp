...
namespace mlr {
bool ReadDataLabelSparseFeatureBinary(
  ...
  while (offset < file_str.size()) {
    int32_t nnz = *reinterpret_cast<int32_t*>(&file_str[offset]);
    offset += sizeof(int32_t);

    int32_t label = *reinterpret_cast<int32_t*>(&file_str[offset]);
    offset += sizeof(int32_t);
    ...
    if (nnz > 0) {
      ...
      (*labels).push_back(label_one_based ? label - 1 : label);
      (*features).push_back(new petuum::ml::SparseFeature<float>(
          feature_ids, feature_vals, feature_dim));
      ++i;
    }
  }
  LOG(INFO)
...
int32_t GetMaxFeatureID(
  ...
  while (offset < file_str.size()) {
    ...
    for (int i = 0; i < nnz; ++i) {
      if (feature_ids_ptr[i] > max_feature_id)
        max_feature_id = feature_ids_ptr[i];
    }
...
void MLREngine::ReadData() {
  ...
  num_train_data_ = train_labels_.size();

  if (FLAGS_perform_test) {
    LOG(INFO) << "read test data";
    for (int i = 0; i < FLAGS_num_test_files_per_client; ++i) {
      int file_index = FLAGS_test_file_st_index
                       + FLAGS_num_test_files_per_client * FLAGS_client_id + i;

      std::string test_file
          = FLAGS_test_file + "." + std::to_string(file_index);

      LOG(INFO) << "reading " << test_file;
    ...
    num_test_data_ = test_labels_.size();
  }
}
...
void MLREngine::Start() {
  ...
  for (int epoch = 0; epoch < num_epochs; ++epoch) {
    step_size = step_size * FLAGS_step_size_decay;
    ...
    while (!workload_mgr.IsEnd()) {
      ...
      mlr_solver->SingleDataSGD(
        *train_features_[data_idx],
        train_labels_[data_idx], step_size);
