...
void LRSGDSolver::SingleDataSGD(
    ...
    int32_t label,
    double sample_lr __attribute__ ((unused)) ) {
...
void LRSGDSolver::ApplyUpdates() {
  ...
  for (int i = 0; i < num_full_rows; ++i) {
    for (int j = 0; j < w_table_num_cols_; ++j) {
      ...
      CHECK_EQ(gradients_[idx], gradients_[idx]) << "nan detected.";
      w_update_batch_[j] = gradients_[idx];
    }
  ...
  if (feature_dim_ % w_table_num_cols_ != 0) {
    ...
    for (int j = 0; j < num_cols_last_row; ++j) {
      ...
      CHECK_EQ(gradients_[idx], gradients_[idx]) << "nan detected.";
      w_update_batch[j] = gradients_[idx];
    }
...
void LRSGDSolver::ReadFreshParams() {
  ...
  for (int i = 0; i < num_full_rows; ++i) {
    ...
    r.CopyToMem(w_cache_vec.data() + i * w_table_num_cols_ * sizeof(float));
  }
