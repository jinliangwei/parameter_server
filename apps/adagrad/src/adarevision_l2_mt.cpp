...
class AdaRevisionLR : public boost::noncopyable {
public:
  AdaRevisionLR();
  ~AdaRevisionLR();
...
private:
  ...
  friend class AdaRevisionLRWorker;
...
class AdaRevisionLRWorker : public petuum::Thread {
public:
  AdaRevisionLRWorker(
      size_t num_workers,
      int32_t my_id,
      int num_epochs,
      AdaRevisionLR &ada_grad,
...
private:
  ...
  void LoadData(const AdaRevisionLR &ada_grad);
  ...
  petuum::ml::DenseFeature<float> *weights_;
  std::vector<float> old_accum_gradients_;
  std::vector<float> gradient_updates_;
};

AdaRevisionLRWorker::AdaRevisionLRWorker(
      size_t num_workers,
      int32_t my_id,
      int num_epochs,
      AdaRevisionLR &ada_grad,
  ...
  old_accum_gradients_.resize(feature_dim_);
...
void AdaRevisionLRWorker::LoadData(const AdaRevisionLR &ada_grad) {
...
void AdaRevisionLRWorker::ApplyUpdates() {
  {
    ...
    for (int i = 0; i < gradient_updates_.size(); ++i) {
      ...
      ada_grad_.z_max_[i] = std::max(ada_grad_.z_[i], ada_grad_.z_max_[i]);
...
void AdaRevisionLRWorker::RefreshWeights() {
  std::lock_guard<std::mutex> lock(ada_grad_.mtx_);
  for (int i = 0; i < feature_dim_; ++i) {
    (*weights_)[i] = (*ada_grad_.weights_)[i];
    old_accum_gradients_[i] = ada_grad_.accum_gradients_[i];
  }
}

void *AdaRevisionLRWorker::operator() () {
...
void AdaRevisionLRWorker::Predict(
    const petuum::ml::AbstractFeature<float>& feature,
    std::vector<float> *result) const {
...
int32_t AdaRevisionLRWorker::ZeroOneLoss(
...
float AdaRevisionLRWorker::CrossEntropyLoss(
    const std::vector<float>& prediction, int32_t label) {
...
float AdaRevisionLRWorker::EvaluateL2RegLoss() const {
  double l2_norm = 0.;
...
AdaRevisionLR::AdaRevisionLR() { }

AdaRevisionLR::~AdaRevisionLR() {
  if (weights_) delete weights_;
}

void AdaRevisionLR::Train(const std::string &train_file,
                      int num_epochs,
                      const std::string &test_file) {
  ...
  accum_gradients_.resize(feature_dim_, 0);
  z_.resize(feature_dim_, 1);
  z_max_.resize(feature_dim_, 1);

  AdaRevisionLRWorker *worker[FLAGS_num_worker_threads];

  for (int i = 0; i < FLAGS_num_worker_threads; ++i) {
    worker[i] = new AdaRevisionLRWorker(FLAGS_num_worker_threads,
                                    i,
                                    FLAGS_num_epochs,
                                    *this,
                                    process_barrier);
...
void AdaRevisionLR::DumpModel() { }

void AdaRevisionLR::LoadData(const std::string &train_file,
                         const std::string &test_file) {
...
void AdaRevisionLR::CleanFeatures() {
