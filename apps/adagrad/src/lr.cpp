...
class LR : public boost::noncopyable {
public:
  LR();
  ~LR();
...
void LR::Train(const std::string &train_file,
                      int num_epochs,
                      const std::string &test_file) {
  ...
  for (int epoch = 0; epoch < num_epochs; ++epoch) {
    ...
    for (const auto &datum : data_) {
      ...
      petuum::ml::FeatureScaleAndAdd(-lr * (predict_buff[0] - label),
                                     feature,
                                     weights_);
      num_processed += 1;
    }
...
void LR::LoadData(const std::string &train_file,
                         const std::string &test_file) {
...
int main(int argc, char *argv[]) {
  ...
  LR lr;
  lr.Train(FLAGS_train_file, FLAGS_num_epochs, FLAGS_test_file);
