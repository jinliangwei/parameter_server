...
void AdaGradLRWorker::LoadData(const AdaGradLR &ada_grad) {
  ...
  if (num_test_data > 0) {
    ...
    size_t num_data_per_thread = num_test_data / num_workers_;
    size_t total_data_assigned = num_data_per_thread * num_workers_;
    int32_t cutoff_id = num_workers_ - 1 - (total_data_assigned - num_data);
  ...
  for (int epoch = 0; epoch < num_epochs_; ++epoch) {
    ...
    if (epoch % FLAGS_num_epochs_per_eval == 0) {
...

      {
        ...
        ada_grad_.entropy_loss_ += total_entropy_loss;
        ada_grad_.zero_one_loss_ += total_zero_one_loss;
      }
      ...
      if (my_id_ == 0) {
        ...
        loss_stream << "Train "
                    ...
                    << ada_grad_.zero_one_loss_ << " "
                    << ada_grad_.entropy_loss_
                    << std::endl;
        loss_stream.close();
