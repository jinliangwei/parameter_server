...
//typedef petuum::DenseRow<float> DenseRowFloat;
typedef petuum::DenseRowFloat16<float> DenseRowFloat;
...
void SgdElement(
  ...
  // Read R(:,j) from Petuum PS
  {
    ...
    const auto& Rj_row = R_table.Get<DenseRowFloat>(j, &Rj_acc);
  ...
  for (int ii = col_begin; ii < col_end; ++ii) {
    {
      petuum::RowAccessor Rj_acc;
      const auto& Rj_row
          = R_table.Get<DenseRowFloat>(ii, &Rj_acc);
      Rj_row.CopyToVector(Rj_cache);
    }
    auto &Rj = *Rj_cache;
...
int main(int argc, char *argv[]) {
  ...
  petuum::PSTableGroup::RegisterRow<DenseRowFloat>(0);
  petuum::PSTableGroup::RegisterRow<petuum::DenseRow<int64_t> >(1);
  petuum::PSTableGroup::RegisterRow<petuum::DenseRow<float> >(2);
  ...
  table_config.table_info.row_type = 2;
