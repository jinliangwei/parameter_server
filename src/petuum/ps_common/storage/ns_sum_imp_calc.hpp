...
#include <petuum/ps_common/storage/ns_abstract_imp_calc.hpp>
...
public:
  ...
  virtual double ApplyDenseBatchIncGetReImportance(
      AbstractStore<V> *store,
      const void* update_batch, int32_t index_st, int32_t num_updates) const;

  virtual double GetReImportance(
...
template<typename V>
double NSSumImpCalc<V>::ApplyIncGetReImportance(
    AbstractStore<V> *store,
    int32_t column_id, const void *update) const {
  V type_update = *(reinterpret_cast<const V*>(update));
  V val = store->Get(column_id);

  double importance = (double(val) == 0) ? double(type_update)
                      : double(type_update) / double(val);
...
template<typename V>
double NSSumImpCalc<V>::ApplyBatchIncGetReImportance(
    AbstractStore<V> *store, const int32_t *column_ids,
    const void* update_batch, int32_t num_updates) const {
  ...
  for (int i = 0; i < num_updates; ++i) {
    ...
    double importance = (double(val) == 0) ? double(update_array[i])
...
template<typename V>
double NSSumImpCalc<V>::ApplyDenseBatchIncGetReImportance(
    AbstractStore<V> *store,
    const void* update_batch, int32_t index_st, int32_t num_updates) const {
...
template<typename V>
double NSSumImpCalc<V>::GetReImportance(
    int32_t column_id, const void *update, const void *value) const {
...
template<typename V>
double NSSumImpCalc<V>::GetDenseAccumReImportance(
      const AbstractStore<V> *store,
      const void *update_batch, int32_t index_st,
      int32_t num_updates) const {
  const V *update_array = reinterpret_cast<const V*>(update_batch);
