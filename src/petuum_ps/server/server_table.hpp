...
struct CandidateServerRow {
  ...
  CandidateServerRow(RowId _row_id,
...
public:
  ...
  ServerRow *FindRow(RowId row_id);

  ServerRow *CreateRow (RowId row_id);

  bool RegisterRowSet(const RowId *row_id_vec, size_t num_row_ids,
                      int32_t client_id);
  void GetBulkInitRows(std::vector<std::vector<RowId>>* row_id_vec);
  bool ApplyRowOpLog (RowId row_id, const int32_t *column_ids,
                      const void *updates, int32_t num_updates);

  void RowSent(RowId row_id, ServerRow *row, size_t num_clients);

  size_t GetNumRows() const { return storage_.size(); }
...
private:
  ...
  boost::unordered_map<RowId, ServerRow*> storage_;

  // used for appending rows to buffs
  boost::unordered_map<RowId, ServerRow*>::iterator row_iter_;
