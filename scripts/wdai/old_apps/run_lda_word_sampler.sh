...
doc_topic_table_process_cache_capacity=500000
...
client_id=0
echo num_unique_host = $num_unique_hosts
for ip in $unique_host_list; do
  ...
  echo GLOG_logtostderr=true GLOG_v=-1 GLOG_minloglevel=0 GLOG_vmodule="" \
    ...
    --doc_topic_table_process_cache_capacity $doc_topic_table_process_cache_capacity \
    --client_id $client_id &
  #ssh $ssh_options $ip \
